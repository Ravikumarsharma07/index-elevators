  ...
  return (
    ...
    >
      ...
      <div className="px-3 pb-4 text-secondary ">
        <div className="flex-between pb-3 md:h-[60px]">
          <h3 className="text-[19px] font-sans font-semibold">
            {category.name}
          </h3>
          <p className="text-[16px] text-nowrap text-primary">Starting from 1000$</p>
        </div>
        <p className="text-secondary/70 text-[16px] font-serif pb-1">{category.description}</p>
        <div className="flex-between w-max text-accent">
        <p className="group-hover:underline font-semibold text-accent">View all products</p>
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
       ...
       <DialogHeader className="p-4 px-6">
           <DialogTitle className="text-xl sm:text-2xl text-start font-bold text-secondary">{category.name}</DialogTitle>
         </DialogHeader>
            ...
            {category.products.map((product, index) => (
              ...
              >
                ...
                <div className="flex-1 space-y-4 ">
                  <div>
                    <h3 className="text-xl font-bold font-serif text-gray-900"> {String(index + 1)}. {product.name}</h3>
